...
public class BSMParser implements Stage<List<PCAPPacket>, List<BSM>> {
...
//					int unknownContextLen = Integer.parseInt(lenHex,16);
    ...
    @Override
    public StageResult<List<BSM>> process(StageResult<List<PCAPPacket>> input) throws StageException {
        log.trace("Entering Parse BSM Step...");
        List<PCAPPacket> packets = input.get();
        log.debug("Processing " + packets.size() + " PCAPPackets for BSM messages.");
        List<BSM> parsed = new ArrayList<>();
        BSMValidator validator = new BSMValidator();

        for (PCAPPacket packet : packets) {
            BSM data = parse(packet.getData());
            if (data != null && validator.validate(data)) {
                parsed.add(data);
            } else {
                log.warn("BSM failed validation, skipping...");
            }
        }

        log.trace("Exiting Parse BSM Step...");

        return new GenericStageResult<>(parsed);
    }
}
